...
BbvaAPI.BASE_URL = 'https://api.ecommercebbva.com';
BbvaAPI.API_VERSION = '/v1/';
BbvaAPI.SANDBOX_URL = 'https://sand-api.ecommercebbva.com';
BbvaAPI.SANDBOX_API_VERSION = '/v1/';


function BbvaAPI(merchantId, privateKey, publicIp, isProductionReady) {
    if (!publicIp) {
        throw new Error("Public Ip can't be null or empty");
    }
    ...
    this.setPublicIp = function (publicIp) {
        this.publicIp = publicIp;
        this._reDefine();
    };
...
BbvaAPI.prototype.define = function (baseData) {
...
var sendRequest = function (data) {
    var baseUrl = data.isSandbox ? BbvaAPI.SANDBOX_URL + BbvaAPI.SANDBOX_API_VERSION : BbvaAPI.BASE_URL + BbvaAPI.API_VERSION;
...
var sendStoreRequest = function (data) {
    var baseUrl = data.isSandbox ? BbvaAPI.SANDBOX_URL + "/" : BbvaAPI.BASE_URL + "/";
    const url = baseUrl + urlUtils.escapeBrackets(data.apiUrl);
    const options = {
        auth: data.privateKey + ':',
        method: data.requestData.method || 'GET',
